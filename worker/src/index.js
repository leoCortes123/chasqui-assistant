// ---------------------------------------------------------------------------
// Chasqui Assistant — worker de tareas asíncronas (§2.2 puntos 3 y 4)
//
...
const pool = new Pool({
  ...
  application_name: 'chasqui-assistant-worker',
});
  ...
  for (;;) {
    ...
    try {
      const { rows } = await pool.query(
        "SELECT current_database() AS db, config_txt('nombre_clinica', 'Chasqui Assistant') AS clinica",
      );
      log.info(`conectado a la base '${rows[0].db}' (${rows[0].clinica})`);
      return true;
    } catch (err) {
      intento += 1;
