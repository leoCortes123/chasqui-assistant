...
//
//   telegram   — el personal. Ya funciona.
//   whatsapp   — el público. Se conecta en la fase 3 (Meta Cloud API).
...
export async function responder(db, conversacion, texto, botones = [], { markdown = false } = {}) {
  ...
  if (canal === 'telegram') {
    ...
    // Fase 3. Hasta que exista el transporte, no se pierde el mensaje: queda
    // en `mensaje` y el panel lo muestra, que es como se descubre que faltaba
    // esto en vez de que el cliente se quede esperando en silencio.
    envio = { ok: false, motivo: 'canal_whatsapp_sin_configurar' };
  }
