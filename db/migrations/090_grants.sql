-- =====================================================================
-- Chasqui Assistant — 090_grants.sql
-- Roles de base de datos y append-only real (§2.2.8).
--
-- La aplicación (n8n, worker, portal) se conecta como `chasqui_assistant_app`,
...
DO $$
BEGIN
  IF NOT EXISTS (SELECT 1 FROM pg_roles WHERE rolname = 'chasqui_assistant_app') THEN
    -- La contraseña se fija desde docker-compose (ver .env). Aquí sólo el rol.
    CREATE ROLE chasqui_assistant_app LOGIN;
  END IF;
  IF NOT EXISTS (SELECT 1 FROM pg_roles WHERE rolname = 'chasqui_assistant_lectura') THEN
    CREATE ROLE chasqui_assistant_lectura LOGIN;   -- reportes / BI, sólo lectura
  END IF;
END $$;

DO $$
BEGIN
  EXECUTE format('GRANT CONNECT ON DATABASE %I TO chasqui_assistant_app, chasqui_assistant_lectura',
                 current_database());
END $$;

GRANT USAGE ON SCHEMA public TO chasqui_assistant_app, chasqui_assistant_lectura;

GRANT SELECT, INSERT, UPDATE, DELETE ON ALL TABLES IN SCHEMA public TO chasqui_assistant_app;
GRANT USAGE, SELECT ON ALL SEQUENCES IN SCHEMA public TO chasqui_assistant_app;
GRANT EXECUTE ON ALL FUNCTIONS IN SCHEMA public TO chasqui_assistant_app;

GRANT SELECT ON ALL TABLES IN SCHEMA public TO chasqui_assistant_lectura;
...
BEGIN
  FOREACH t IN ARRAY tablas LOOP
    IF to_regclass('public.' || t) IS NOT NULL THEN
      EXECUTE format('REVOKE UPDATE, DELETE, TRUNCATE ON %I FROM chasqui_assistant_app', t);
      EXECUTE format('GRANT SELECT, INSERT ON %I TO chasqui_assistant_app', t);
      EXECUTE format('REVOKE ALL ON %I FROM chasqui_assistant_lectura', t);
      EXECUTE format('GRANT SELECT ON %I TO chasqui_assistant_lectura', t);
    END IF;
  END LOOP;
END $$;
...
REVOKE DELETE, TRUNCATE ON telegram_update FROM chasqui_assistant_app;
...
ALTER DEFAULT PRIVILEGES IN SCHEMA public
  GRANT SELECT, INSERT, UPDATE, DELETE ON TABLES TO chasqui_assistant_app;
ALTER DEFAULT PRIVILEGES IN SCHEMA public
  GRANT USAGE, SELECT ON SEQUENCES TO chasqui_assistant_app;
ALTER DEFAULT PRIVILEGES IN SCHEMA public
  GRANT EXECUTE ON FUNCTIONS TO chasqui_assistant_app;
ALTER DEFAULT PRIVILEGES IN SCHEMA public
  GRANT SELECT ON TABLES TO chasqui_assistant_lectura;
