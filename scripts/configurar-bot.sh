#!/usr/bin/env bash
# =====================================================================
# Chasqui Assistant — configurar-bot.sh
...
RUTA_WEBHOOK="${WEBHOOK_URL%/}/webhook/chasqui-assistant-telegram"
...
if [ -n "$sede" ]; then
  ...
  echo "    Pantalla en la clínica: ${WEB_PUBLIC_URL:-http://localhost:3200}/pantalla/${sede}"
  if [ -n "$publica" ]; then
    echo
