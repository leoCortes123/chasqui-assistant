#!/bin/sh
# =============================================================================
# Chasqui Assistant — Respaldo automático de la base de datos
# -----------------------------------------------------------------------------
...
BASE="${PGDATABASE:-chasqui_assistant}"
...
hacer_respaldo() {
    ...
    destino="$BACKUP_DIR/chasqui-assistant-${BASE}-${marca}.dump"
...
limpiar_antiguos() {
    ...
    for viejo in $(find "$BACKUP_DIR" -maxdepth 1 -type f -name 'chasqui-assistant-*.dump' -mtime "+$RETENCION_DIAS" 2>/dev/null); do
        rm -f "$viejo"
    ...
    log "Respaldos disponibles: $(find "$BACKUP_DIR" -maxdepth 1 -type f -name 'chasqui-assistant-*.dump' | wc -l | tr -d ' ')"
}
...
if [ -z "$(find "$BACKUP_DIR" -maxdepth 1 -type f -name "chasqui-assistant-${BASE}-$(date '+%Y%m%d')-*.dump" 2>/dev/null)" ]; then
