#!/usr/bin/env bash
# =====================================================================
# Chasqui Assistant — importar-n8n.sh
...
TMP_REMOTO="/tmp/chasqui-assistant-credencial.json"

echo "==> Creando la credencial de Postgres en n8n"
docker compose exec -T n8n sh -c "cat > ${TMP_REMOTO}" <<JSON
[
  {
    "id": "chasqui-assistant-postgres",
    "name": "Chasqui Assistant Postgres",
    "type": "postgres",
    "data": {
      "host": "db",
      "port": 5432,
      "database": "${POSTGRES_DB}",
      "user": "chasqui_assistant_app",
