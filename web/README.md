# Chasqui Assistant — Portal web y pantalla de turnos

Aplicación Next.js (App Router, TypeScript) de Chasqui Assistant. Incluye el health
...
- Node 22
- PostgreSQL con el esquema de Chasqui Assistant ya cargado
...
```bash
DATABASE_URL=postgresql://chasqui_assistant_app:CLAVE@localhost:5432/chasqui_assistant
```
...
| `NOMBRE_CLINICA` | Nombre que se muestra en la pantalla cuando no hay nadie en atención y en la cabecera del portal. | `Chasqui Assistant` |
...
cd web
npm install
DATABASE_URL=postgresql://chasqui_assistant_app:CLAVE@localhost:5432/chasqui_assistant npm run dev
```
...
```bash
psql -d chasqui_assistant -tAc "SELECT id, nombre FROM sede WHERE activa"
```
