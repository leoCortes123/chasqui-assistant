...
const global_ = globalThis as unknown as { __chasqui_assistantPool?: Pool };

export function obtenerPool(): Pool {
  if (!global_.__chasqui_assistantPool) {
    const pool = new Pool({
      connectionString: urlConsultas(),
      max: MAX_CONEXIONES,
      idleTimeoutMillis: 30_000,
      connectionTimeoutMillis: 5_000,
      application_name: 'chasqui-assistant-web',
    });
    ...
    global_.__chasqui_assistantPool = pool;
  }
  return global_.__chasqui_assistantPool;
}
...
export async function crearClienteEscucha(): Promise<Client> {
  const cliente = new Client({
    ...
    application_name: 'chasqui-assistant-web-listen',
  });
